...
if [ $stage -le 2 ]; then
    for wav_type in array head; do
        bad_utts=conf/bad_utts_$wav_type
    ...
    for wav_type in array head; do
        ...
        for spk in $speakers; do
            ...
            utils/subset_data_dir_tr_cv.sh --cv-spk-list "$tmp/$spk" $datadir/$wav_type \
                                        $split/${spk}/train $split/$spk/test
            utils/subset_data_dir.sh --utt-list $tmp/single.utts \
                                    $split/$spk/test $split/$spk/test_single
            utils/subset_data_dir.sh --utt-list $tmp/single.utts \
                                    $split/$spk/test $split/$spk/test_single_nolimit
            utils/subset_data_dir.sh --utt-list $tmp/multi.utts \
                                    $split/$spk/test $split/$spk/test_multi
        done
    done
fi
...
if [ $stage -le 3 ]; then
    for wav_type in array head; do
        ...
        local/multi_torgo_prepare_lm.sh $datadir/$wav_type || exit 1
